...
def create_nanodet_dataset(mindrecord_file, batch_size, repeat_num, device_num=1, rank=0,
    ...
    # 返回 原始数据增强后的image, 筛选后的bboxes(正样本的Anchor 格式:中心点宽高), 正样本类别, 正样本数量
    # "annotation": {"type": "int32", "shape": [-1, 5]
    ...
    # ds包含 原始数据增强后的image, 筛选后的bboxes(正样本的Anchor 格式:中心点宽高), 正样本类别, 正样本数量
    ...
    # ds第二次处理
    # input_columns=["image"] 对image进行处理
    # trans = [color_adjust_op, normalize_op, change_swap_op]
    ...
    # 返回dataset
    # 我最后返回的 output_columns = ["image", "box", "label", "num_match"]
